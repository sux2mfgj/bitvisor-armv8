...
static u32 gicc_read_32(struct gic_dev *gic, size_t offset)
{
    return *(volatile u32*)(gic->cpu_mmio_base + offset);
}

static void gicc_write_32(struct gic_dev *gic, size_t offset, u32 value)
{
    *(volatile u32 *)(gic->cpu_mmio_base + offset) = value;
}

static u32 gicd_read_32(struct gic_dev *gic, size_t offset)
{
    return *(volatile u32 *)(gic->dist_mmio_base + offset);
}

static void gicd_write_32(struct gic_dev *gic, size_t offset, u32 value)
{
    *(volatile u32*)(gic->dist_mmio_base + offset) = value;
}

void gic_init_controller(struct gic_dev *gic)
{
    // stop distributer
...
static void gic_fdt_init (struct fdt_node* node)
{
    int res;
    u64 gicd_base, gicc_base;

    res = fdt_get_reg_value(node, FDT_REG_INDEX_GIC_GICD, FDT_REG_ADDRESS, &gicd_base);
    if (res) {
        panic("oh no");
    }

    res = fdt_get_reg_value(node, FDT_REG_INDEX_GIC_GICC, FDT_REG_ADDRESS, &gicc_base);
    if (res) {
        panic("oh no");
    }
